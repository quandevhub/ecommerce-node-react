...
test.describe('Cart Page', () => {
    ...
    test('giỏ hàng rỗng → Thanh toán bị disabled và tổng = 0', async ({ page }) => {
    ...
    test('thêm sản phẩm → hiển thị trong giỏ hàng với quantity = 1', async ({ page }) => {
    ...
    test('thêm cùng sản phẩm 2 lần → quantity = 2 và chỉ có 1 row', async ({ page }) => {
        await page.goto(`${BASE}/`)
        await waitForProducts(page)
    ...
    test('click - khi quantity = 1 → sản phẩm bị xóa khỏi giỏ', async ({ page }) => {
        await addFirstProductToCart(page)
        await goToCart(page)
    ...
    test('click Xoa → sản phẩm biến mất khỏi danh sách', async ({ page }) => {
        ...
        await page.getByRole('button', { name: 'Xoa', exact: true }).first().click()
    ...
    test('checkout khi chưa login → redirect sang /login?redirect=/cart', async ({ page }) => {
        await addFirstProductToCart(page)
        await goToCart(page)
    ...
    test('checkout thành công → banner xanh và giỏ hàng bị xóa', async ({ page }) => {
        ...
        await expect(page.locator('.bg-green-50')).toContainText('Đặt hàng thành công')
        await expect(page.locator('.space-y-4 > div')).toHaveCount(0)
        await expect(page.getByText(/Tong so luong: 0/)).toBeVisible()
    })

    test('checkout server lỗi → banner đỏ hiển thị lỗi kết nối', async ({ page }) => {
        await loginAndAddProduct(page)
        await goToCart(page)
