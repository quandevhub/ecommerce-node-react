...
test.describe('Login Page', () => {
    ...
    test('sai mật khẩu → banner đỏ từ API', async ({ page }) => {
        ...
        await expect(page.locator('.bg-red-50')).toContainText('Email hoặc mật khẩu không đúng')
    })
    ...
    test('login thành công → header hiển thị email và nút Đăng xuất', async ({ page }) => {
        await page.getByLabel('Email').fill(VALID_EMAIL)
        await page.getByLabel('Mật khẩu').fill(VALID_PASSWORD)
        await page.getByRole('button', { name: 'Đăng nhập' }).click()
    ...
    test('login với ?redirect=/cart → redirect về /cart sau khi login', async ({ page }) => {
