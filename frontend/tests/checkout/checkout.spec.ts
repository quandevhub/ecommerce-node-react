...
test.describe('Checkout Flow', () => {
    ...
    test('chưa login, click Thanh toán → redirect /login?redirect=/cart', async ({ page }) => {
    ...
    test('login với ?redirect=/cart → auto-redirect về /cart', async ({ page }) => {
    ...
    test('button hiển thị "Đang xử lý..." và bị disabled trong khi API đang chạy', async ({ page }) => {
        ...
        await page.route('**/api/orders', async route => {
            ...
            await route.fulfill({
                ...
                body: JSON.stringify({ message: 'Order created successfully', data: {} }),
            })
        })
    ...
    test('double-click Thanh toán → chỉ gọi API 1 lần', async ({ page }) => {
        ...
        await page.getByRole('button', { name: 'Đang xử lý...' }).click({ force: true })
    ...
    test('API 500 không có message → banner đỏ với STATUS_MESSAGES[500]', async ({ page }) => {
        await loginAndAddProduct(page)
        await goToCart(page)
        ...
        await expect(page.locator('.bg-red-50')).toContainText('Lỗi máy chủ, vui lòng thử lại sau')
    })

    test('API trả về message tùy chỉnh → banner đỏ hiển thị đúng message đó', async ({ page }) => {
        await loginAndAddProduct(page)
        await goToCart(page)
        ...
        await page.route('**/api/orders', route =>
            route.fulfill({
                status: 422,
                contentType: 'application/json',
                body: JSON.stringify({ message: 'Sản phẩm không đủ số lượng trong kho' }),
            })
        )
        ...
        await expect(page.locator('.bg-red-50')).toBeVisible()
        await expect(page.locator('.bg-red-50')).toContainText('Sản phẩm không đủ số lượng trong kho')
    })
})
